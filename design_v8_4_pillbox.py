#!/usr/bin/env python3
"""v8.4 — the full credit-card pillbox with the validated v7.4 side-bolt lock.
...
CARRIED OVER FROM v7.4 UNCHANGED (validated by hand, not redesigned)
  - Blade bolt cross-section 5.0 (X) x 10.0 (Z). Sliding clearance was 0.25mm/face on the
    coupons; v8.4 opens it to 0.30 because the printed v8.0 was still tight in the hand.
  - Detent in the HOUSING (v7.3): a 1.0 x 15mm cantilever fin in the drawer's front block,
    ...
    the crossover is 0.25mm, the low-strain number v7.3's FEA sized, and v8.4 holds it
  ...
  - Thumb PAD, not a thumbnail tab: FULL bolt height, 2.5mm proud, all as vertical ribs so
    the bolt slices bridge-free. v8.4 lengthens it 14 -> 18mm along Y with 5 ridges.
  - Bolt tip FLUSH at LOCKED — in v8.4, flush with the sleeve's plain 54.0 outer face.
...
channel — bridge droop, not stringing). Three changes, all reported with spans below:
  ...
  4. v8.4 GABLES the bore roof. Chamfering the two upper corners and leaving a flat crown
...
DELIBERATE DEVIATIONS FROM SPEC.md — all of them
  ...
  - The +Y wall is 3.0mm where SPEC says 1.7. It has to be: it is the wall the bolt
    ...
    complaint; v8.4 takes the 1.2 out of the cavity instead so the outer face is dead
  ...
  - v8.4: ALL fits were signed off by hand on the v8.3 prints. The only change here is
    deleting the proud boss; every clearance, the detent, the stroke, the dock and the
    gable carry over untouched, re-anchored to the new wall.
...
DELIBERATELY OMITTED — noted, not silently dropped
  ...
  - SPEC's three friction detents and their sleeve receivers. The bolt is v8.4's
...
WHAT CHANGED FROM v8.0 (operator printed it, trimmed and sanded it, and hand-tested it)
  ...
  v8.4 (hand test of the printed v8.1, carrying v8.2's gable forward):
...
Usage:  blender -b --factory-startup --python design_v8_4_pillbox.py [-- --only Bolt]
Writes: print/v8_4_pillbox_plate.stl  (or print/v8_4_pillbox_<part>.stl with --only)
"""
import os
import sys
...
# v8.4: the +Y wall carries the lock, so it is a uniform 3.0mm for its whole area. v8.3
...
# v8.4 anti-wiggle: side 0.20 -> 0.15, ceiling 0.60 -> 0.45. The floor stays 0.20 — the
...
CLR = 0.30                                             # v8.4: 0.25 -> 0.30, still tight
BOLT_T, BOLT_H = 5.0, 10.0
TRAVEL = 5.0
PAD_PROUD, PAD_L = 2.5, 18.0                           # v8.4: 14 -> 18 along Y
RIDGE_P, RIDGE_W, RIDGE_N = 0.5, 1.2, 5                # v8.4: 4 -> 5 ridges
# v8.4: the stroke moves inboard. PAD_TIP_GAP is how far the pad's +Y end sits behind the
...
RIDGE_X0 = 1.2                                         # v8.4: 2.0 -> 1.2, see DR_X0 below
...
# v8.4 shallower inset: the whole bolt/pad column moved 0.80 toward the mouth via
...
# v8.4 GABLE. v8.1 chamfered the bore roof's two upper corners and left a flat crown
...
# --- Y chain -----------------------------------------------------------------------
TIP_LOCKED = BOX_Y                                     # 54.00 flush with the PLAIN face
TIP_OPEN = TIP_LOCKED - TRAVEL                         # 50.20 vs drawer inner wall 50.50
# v8.4: 45.0 -> 44.0. The +Y side of the whole stack moved 1.20 inboard with the cavity,
...
# v8.4 ZERO-SLACK DOCK. v8.2 shared the notch's arc centre, so the knob sat 0.30 shy of
...
# v8.4: moving the stroke inboard drags the pad slot's -Y end down to 25.30, so the fin
...
RELIEF_Y0 = 5.1                                        # v8.4: follows the slot 1.2 inboard
# v8.4: the relief's flat cap is gone. Its ceiling is now the bore's own back gable plane
...
# --- lock boss + hole --------------------------------------------------------------
# v8.4: no boss. There is nothing to add to the outside of the +Y wall any more — the wall
# IS 3.0 thick everywhere — so BOSS_X/Z/CHAMFER are gone along with their 45deg blend ramp.
# v8.4 ASYMMETRIC hole in X. Only ONE of the hole's two X faces is ever loaded: pull the
...
sleeve = bevel(box("Sleeve_V84", 0, BOX_X, 0, BOX_Y, 0, BOX_Z), EDGE_BREAK)
...
# v8.4 envelope assert: nothing may stand proud of the credit-card outline. Checked on the
...
if abs(_sy0) > 1e-4 or abs(_sy1 - BOX_Y) > 1e-4:
    raise SystemExit(f"v8.4: sleeve spans Y {_sy0:.4f}..{_sy1:.4f}, must be 0..{BOX_Y}")
...
# --- DRAWER ------------------------------------------------------------------------
drawer = bevel(box("Drawer_V84", DR_X0, DR_X1, DR_Y0, DR_Y1, DR_Z0, DR_Z1), EDGE_BREAK)
...
# off the bed. v8.1 closed its top with a flat cap, which was itself a 2.5mm bridge; v8.4
...
bolt = bevel(box("Bolt_V84", BOLT_X0, BOLT_X1, TAIL_LOCKED, TIP_LOCKED, BOLT_Z0, BOLT_Z1),
             EDGE_BREAK)
...
def report():
    print("\nv8.4 Y-STACK (mm, along the bolt axis)")
    for label, y, note in [
        ...
        ("sleeve OUTER face", BOX_Y,
         f"uniform {WALL_LOCK} lock wall, dead flat, nothing proud — v8.4"),
    ]:
    ...
    print(f"   LOCKED tip y={TIP_LOCKED:.2f} vs the plain outer face y={BOX_Y:.2f}"
          f" -> flush by {BOX_Y - TIP_LOCKED:+.2f};"
    ...
    print("\nv8.4 LOCKED FREE PLAY (mm of drawer movement, v8.2 -> v8.4)")
    ...
    print(f"   fin {FIN_T} x {_fin_l:.1f} long x {FIN_Z1 - DR_Z0:.2f} tall,"
          f" anchored at y={FIN_Y0:.1f}, free at y={FIN_Y1:.1f} (flipped in v8.4),"
    ...
    print("\nv8.4 BRIDGED FACES (flat span that must bridge, mm)")
...
if ONLY:
    ...
    if not bpy.data.objects:
        raise SystemExit(f"v8.4: no part matches --only {ONLY!r}")
...
for o in bpy.data.objects:
    ...
    if max(abs(x0), abs(x1), abs(y0), abs(y1)) > PLATE_LIMIT:
        raise SystemExit(f"v8.4: {o.name} leaves the +/-{PLATE_LIMIT}mm plate window")
...
for _a in range(len(_parts)):
    for _b in range(_a + 1, len(_parts)):
        ...
        if gap < PART_GAP:
            raise SystemExit(f"v8.4: {_parts[_a].name}/{_parts[_b].name} gap {gap:.2f}"
                             f" < {PART_GAP}mm")

name = f"v8_4_pillbox_{ONLY.lower()}.stl" if ONLY else "v8_4_pillbox_plate.stl"
