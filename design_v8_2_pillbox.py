#!/usr/bin/env python3
"""v8.2 — the full credit-card pillbox with the validated v7.4 side-bolt lock.
...
CARRIED OVER FROM v7.4 UNCHANGED (validated by hand, not redesigned)
  - Blade bolt cross-section 5.0 (X) x 10.0 (Z). Sliding clearance was 0.25mm/face on the
    coupons; v8.2 opens it to 0.30 because the printed v8.0 was still tight in the hand.
  - Detent in the HOUSING (v7.3): a 1.0 x 15mm cantilever fin in the drawer's front block,
    ...
    the crossover is 0.25mm, the low-strain number v7.3's FEA sized, and v8.2 holds it
  ...
  - Thumb PAD, not a thumbnail tab: FULL bolt height, 2.5mm proud, all as vertical ribs so
    the bolt slices bridge-free. v8.2 lengthens it 14 -> 18mm along Y with 5 ridges.
...
channel — bridge droop, not stringing). Three changes, all reported with spans below:
  ...
  4. v8.2 GABLES the bore roof. Chamfering the two upper corners and leaving a flat crown
...
DELIBERATELY OMITTED — noted, not silently dropped
  ...
  - SPEC's three friction detents and their sleeve receivers. The bolt is v8.2's
...
Usage:  blender -b --factory-startup --python design_v8_2_pillbox.py [-- --only Bolt]
Writes: print/v8_2_pillbox_plate.stl  (or print/v8_2_pillbox_<part>.stl with --only)
"""
import os
import sys
...
CLR = 0.30                                             # v8.2: 0.25 -> 0.30, still tight
BOLT_T, BOLT_H = 5.0, 10.0
TRAVEL = 5.0
PAD_PROUD, PAD_L = 2.5, 18.0                           # v8.2: 14 -> 18 along Y
RIDGE_P, RIDGE_W, RIDGE_N = 0.5, 1.2, 5                # v8.2: 4 -> 5 ridges
PAD_TIP_GAP = 3.4
...
# v8.2 GABLE. v8.1 chamfered the bore roof's two upper corners and left a flat crown
...
# v8.2: the relief's flat cap is gone. Its ceiling is now the bore's own back gable plane
...
BOSS_CHAMFER = 1.4
HOLE_CLR_X, HOLE_CLR_Z = 0.45, 0.30                    # v8.2: both +0.05 with CLR
...
sleeve = bevel(box("Sleeve_V82", 0, BOX_X, 0, BOX_Y, 0, BOX_Z), EDGE_BREAK)
...
# --- DRAWER ------------------------------------------------------------------------
drawer = bevel(box("Drawer_V82", DR_X0, DR_X1, DR_Y0, DR_Y1, DR_Z0, DR_Z1), EDGE_BREAK)
...
# off the bed. v8.1 closed its top with a flat cap, which was itself a 2.5mm bridge; v8.2
...
bolt = bevel(box("Bolt_V82", BOLT_X0, BOLT_X1, TAIL_LOCKED, TIP_LOCKED, BOLT_Z0, BOLT_Z1),
             EDGE_BREAK)
...
def report():
    print("\nv8.2 Y-STACK (mm, along the bolt axis)")
    for label, y, note in [
    ...
    print("\nv8.2 BRIDGED FACES (flat span that must bridge, mm)")
...
if ONLY:
    ...
    if not bpy.data.objects:
        raise SystemExit(f"v8.2: no part matches --only {ONLY!r}")
...
for o in bpy.data.objects:
    ...
    if max(abs(x0), abs(x1), abs(y0), abs(y1)) > PLATE_LIMIT:
        raise SystemExit(f"v8.2: {o.name} leaves the +/-{PLATE_LIMIT}mm plate window")
...
for _a in range(len(_parts)):
    for _b in range(_a + 1, len(_parts)):
        ...
        if gap < PART_GAP:
            raise SystemExit(f"v8.2: {_parts[_a].name}/{_parts[_b].name} gap {gap:.2f}"
                             f" < {PART_GAP}mm")

name = f"v8_2_pillbox_{ONLY.lower()}.stl" if ONLY else "v8_2_pillbox_plate.stl"
