#!/usr/bin/env python3
"""v8.0 — the full credit-card pillbox with the validated v7.4 side-bolt lock.
...
CARRIED OVER FROM v7.4 UNCHANGED (validated by hand, not redesigned)
  - Blade bolt cross-section 5.0 (X) x 10.0 (Z), 0.25mm/face sliding clearance in its
    channel. "Fits cleanly into the groove" on the v7.2/v7.3 prints. NOT tightened.
  - Detent in the HOUSING (v7.3): a 1.0 x 15mm cantilever fin in the drawer's front block,
    bump 0.75mm proud, into two dimples in the bolt's back face — LOCKED 0.7, OPEN 0.45.
    Fin deflection at the crossover is 0.25mm, the low-strain number v7.3's FEA sized.
  ...
  - Thumb PAD, not a thumbnail tab: 14mm along Y, FULL bolt height, 2.5mm proud, 4 ridges,
    all as vertical ribs so the bolt slices bridge-free.
...
channel — bridge droop, not stringing). Three changes, all reported with spans below:
  1. The blade's long edges are chamfered 2.0 (front pair) and 2.5 (back top) at 45deg.
     ...
     only be filled to within 0.25mm of the blade.
  2. The channel's upper inside corners are chamfered 2.1 (front) and 2.6 (back), so the
     flat span narrows from 5.5mm to 0.80mm before it ever bridges.
  3. The fin relief and the thumb-pad slot no longer overlap in Y. Overlapped (the naive
     ...
     Separated, the worst span in the drawer is 4.40mm. Separation costs bolt length: the
     bump must stay under the blade at both positions. That cost is a benefit — see the
     guide-length note below; the blade is 44.0mm.
...
DELIBERATE DEVIATIONS FROM SPEC.md — all of them
  ...
  - Drawer 80.95 x 50.00 x 15.40 (SPEC "approximately 82 x 50.2 x 15.0"). DERIVED from the
    cavity and SPEC's clearances (0.20 each side, 0.20 below, 0.60 to the ceiling); the
    length falls out of the front-block depth the mechanism needs.
  ...
  - Sleeve bolt hole is 0.40mm/face clear in X, 0.25mm/face in Z. X accumulates the
...
Usage:  blender -b --factory-startup --python design_v8_0_pillbox.py [-- --only Bolt]
Writes: print/v8_0_pillbox_plate.stl  (or print/v8_0_pillbox_<part>.stl with --only)
"""
import os
import sys
...
TRAVEL = 5.0
PAD_PROUD, PAD_L = 2.5, 14.0
RIDGE_P, RIDGE_W, RIDGE_N = 0.5, 1.2, 4
PAD_TIP_GAP = 3.4
...
CHAN_CF, CHAN_CB = 2.1, 2.6                            # channel roof: front, back corners
...
TAIL_LOCKED = TIP_LOCKED - BOLT_L                      # 11.20
...
FIN_Y0 = FIN_Y1 - FIN_LEN                              # 14.00 free edge
FIN_Z1 = 13.00                                         # 1.3 below the relief cap
BUMP_Y, BUMP_R, BUMP_PROUD = 17.0, 1.1, 0.75
DIMPLE_R = 1.1
...
BOSS_CHAMFER = 1.4
HOLE_CLR_X, HOLE_CLR_Z = 0.40, 0.25
HOLE_X0, HOLE_X1 = BOLT_X0 - HOLE_CLR_X, BOLT_X1 + HOLE_CLR_X   # 4.60 .. 10.40
HOLE_Z0, HOLE_Z1 = BOLT_Z0 - HOLE_CLR_Z, BOLT_Z1 + HOLE_CLR_Z   # 4.05 .. 14.55
HOLE_CH = 2.2                                          # print-ceiling corners of the hole
...
sleeve = bevel(box("Sleeve_V80", 0, BOX_X, 0, BOX_Y, 0, BOX_Z), EDGE_BREAK)
...
# --- DRAWER ------------------------------------------------------------------------
drawer = bevel(box("Drawer_V80", DR_X0, DR_X1, DR_Y0, DR_Y1, DR_Z0, DR_Z1), EDGE_BREAK)
...
bolt = bevel(box("Bolt_V80", BOLT_X0, BOLT_X1, TAIL_LOCKED, TIP_LOCKED, BOLT_Z0, BOLT_Z1),
             EDGE_BREAK)
...
def report():
    print("\nv8.0 Y-STACK (mm, along the bolt axis)")
    for label, y, note in [
    ...
    print("\nv8.0 DETENT")
    print(f"   fin {FIN_T} x {FIN_Y1 - FIN_Y0:.1f} long x {FIN_Z1 - DR_Z0:.2f} tall,"
    ...
    print(f"   bump crown x={FIN_X0 - BUMP_PROUD:.2f} vs blade back face x={BOLT_X1:.2f}"
          f"  -> deflection at the crossover {BOLT_X1 - (FIN_X0 - BUMP_PROUD):.2f}")
    ...
    print("\nv8.0 BRIDGED FACES (flat span that must bridge, mm)")
...
if ONLY:
    ...
    if not bpy.data.objects:
        raise SystemExit(f"v8.0: no part matches --only {ONLY!r}")
...
for o in bpy.data.objects:
    ...
    if max(abs(x0), abs(x1), abs(y0), abs(y1)) > PLATE_LIMIT:
        raise SystemExit(f"v8.0: {o.name} leaves the +/-{PLATE_LIMIT}mm plate window")
...
for _a in range(len(_parts)):
    for _b in range(_a + 1, len(_parts)):
        ...
        if gap < PART_GAP:
            raise SystemExit(f"v8.0: {_parts[_a].name}/{_parts[_b].name} gap {gap:.2f}"
                             f" < {PART_GAP}mm")

name = f"v8_0_pillbox_{ONLY.lower()}.stl" if ONLY else "v8_0_pillbox_plate.stl"
