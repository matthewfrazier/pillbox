#!/usr/bin/env python3
"""v8.5 — the full credit-card pillbox with the validated v7.4 side-bolt lock.
...
Usage:  blender -b --factory-startup --python design_v8_5_pillbox.py [-- --only Bolt]
Writes: print/v8_5_pillbox_plate.stl  (or print/v8_5_pillbox_<part>.stl with --only)
"""
import os
import sys
...
for _name, _got, _want in [
    ...
    if abs(_got - _want) > 1e-6:
        raise SystemExit(f"v8.5: FROZEN housing value {_name} is {_got}, must stay {_want}")
...
if abs(TIP_LOCKED - 54.40) > 0.05:
    raise SystemExit(f"v8.5: tip at LOCKED {TIP_LOCKED:.3f}, must be 54.40 +/-0.05")
if TIP_OPEN > 49.05:
    raise SystemExit(f"v8.5: tip at OPEN {TIP_OPEN:.3f}, must be <= 49.05")
if abs((DIMPLE_OPEN_Y - DIMPLE_LOCK_Y) - TRAVEL) > 1e-9:
    raise SystemExit("v8.5: dimple spacing must equal the stroke")
if abs(PAD_Y1_LOCKED - CAV_Y1) > 1e-6:
    raise SystemExit(f"v8.5: pad at LOCKED ends {PAD_Y1_LOCKED:.3f}, must bear on"
                     f" the sleeve wall at {CAV_Y1:.3f}")
if _TAIL_OPEN < CH_Y0 + 0.5:
    raise SystemExit(f"v8.5: tail at OPEN {_TAIL_OPEN:.3f} is under 0.5 clear of the"
                     f" bore end {CH_Y0:.2f}")
if _PAD_Y0_OPEN < SLOT_Y0 or PAD_Y1_LOCKED - TRAVEL > SLOT_Y1:
    raise SystemExit("v8.5: pad leaves its slot somewhere in the stroke")
if DIMPLE_LOCK_Y - _TAIL_OPEN < 2.5:
    raise SystemExit("v8.5: LOCKED dimple too near the blade tail")
...
sleeve = bevel(box("Sleeve_V85", 0, BOX_X, 0, BOX_Y, 0, BOX_Z), EDGE_BREAK)
...
if abs(_sy0) > 1e-4 or abs(_sy1 - BOX_Y) > 1e-4:
    raise SystemExit(f"v8.5: sleeve spans Y {_sy0:.4f}..{_sy1:.4f}, must be 0..{BOX_Y}")
...
# --- DRAWER ------------------------------------------------------------------------
drawer = bevel(box("Drawer_V85", DR_X0, DR_X1, DR_Y0, DR_Y1, DR_Z0, DR_Z1), EDGE_BREAK)
...
bolt = bevel(box("Bolt_V85", BOLT_X0, BOLT_X1, TAIL_LOCKED, TIP_LOCKED, BOLT_Z0, BOLT_Z1),
             EDGE_BREAK)
...
def report():
    print("\nv8.5 Y-STACK (mm, along the bolt axis)")
    for label, y, note in [
    ...
    print("\nv8.5 LOCKED FREE PLAY (mm of drawer movement, v8.2 -> v8.5)")
    ...
    print("\nv8.5 BRIDGED FACES (flat span that must bridge, mm)")
...
if ONLY:
    ...
    if not bpy.data.objects:
        raise SystemExit(f"v8.5: no part matches --only {ONLY!r}")
...
for o in bpy.data.objects:
    ...
    if max(abs(x0), abs(x1), abs(y0), abs(y1)) > PLATE_LIMIT:
        raise SystemExit(f"v8.5: {o.name} leaves the +/-{PLATE_LIMIT}mm plate window")
...
for _a in range(len(_parts)):
    for _b in range(_a + 1, len(_parts)):
        ...
        if gap < PART_GAP:
            raise SystemExit(f"v8.5: {_parts[_a].name}/{_parts[_b].name} gap {gap:.2f}"
                             f" < {PART_GAP}mm")

name = f"v8_5_pillbox_{ONLY.lower()}.stl" if ONLY else "v8_5_pillbox_plate.stl"
