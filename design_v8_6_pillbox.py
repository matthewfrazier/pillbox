#!/usr/bin/env python3
"""v8.6 — v8.5's bolt exactly, with the version debossed on it.

v8.5's geometry, unchanged. The drawer and sleeve are the printed, immutable v8.4 parts and
the bolt's fits were confirmed in the hand, so a dimensional change here would be a
regression and nothing else. The only difference in the mesh is a 0.6mm recess reading
"v8.6". It is cut HERE, in the design source, rather than onto the exported STL afterwards,
because the project builds print/{version}_pillbox_plate.stl from the tagged script: a mark
applied downstream is gone at the next refresh. A guard block below proves the rest of the
bolt did not move — same bounding box to 1e-4, volume down by the mark's own 7.966mm3 and
by nothing else.
...
WHERE THE MARK GOES
  ...
  4.0 x 6.5. "v8.6" at the 4.0mm cap is 4.11 x 9.50, so with 1mm margins all round only two
  faces can hold it at all, and +X is the blade's bearing and detent side — the one face the
  placement rules forbid outright. That leaves -Z: the land beside the thumb ridges, where
  the pad refills the blade's front chamfers over its own 18mm. It only fits ACROSS that
  land's long axis (unrotated the land is 5.0mm wide after margins), so the mark is rotated
  90deg and reads along the bolt. Centred, it clears the ridge feet and the blade's
  chamfered back edge by ~1.4mm and both ends of the pad by ~4mm.
  ...
  "v8.6 14ag26" is 27.9mm and fits nowhere on this bolt at the legibility floors, so the mark
  is the version alone. The datecode belongs on a part with room for it.

THE POSE STAYS, THE MARK GETS DEEPER
  ...
  perimeters on layers 1, 2 and 3, closed over by layer 4. Cost, stated plainly: those
  recess ceilings add 13.3mm2 to printcheck's overhang count. They are the recess itself,
  a stroke-wide roof three layers up, not a span.
...
Print: all three are posed supportless, but v8.6's DEFAULT PLATE IS THE BOLT ALONE — it is
the only part that changes, and the housing in the operator's hand is the same v8.4 drawer
and sleeve this script would re-emit byte for byte, so "slice v8.6" reprints exactly what is
...
Usage:  blender -b --factory-startup --python design_v8_6_pillbox.py [-- --only Bolt]
Writes: print/v8_6_pillbox_plate.stl — the marked bolt (or print/v8_6_pillbox_<part>.stl
...
for _name, _got, _want in [
    ...
    if abs(_got - _want) > 1e-6:
        raise SystemExit(f"v8.6: FROZEN housing value {_name} is {_got}, must stay {_want}")
...
if abs(TIP_LOCKED - 54.40) > 0.05:
    raise SystemExit(f"v8.6: tip at LOCKED {TIP_LOCKED:.3f}, must be 54.40 +/-0.05")
if TIP_OPEN > 49.05:
    raise SystemExit(f"v8.6: tip at OPEN {TIP_OPEN:.3f}, must be <= 49.05")
if abs((DIMPLE_OPEN_Y - DIMPLE_LOCK_Y) - TRAVEL) > 1e-9:
    raise SystemExit("v8.6: dimple spacing must equal the stroke")
if abs(PAD_Y1_LOCKED - CAV_Y1) > 1e-6:
    raise SystemExit(f"v8.6: pad at LOCKED ends {PAD_Y1_LOCKED:.3f}, must bear on"
                     f" the sleeve wall at {CAV_Y1:.3f}")
if _TAIL_OPEN < CH_Y0 + 0.5:
    raise SystemExit(f"v8.6: tail at OPEN {_TAIL_OPEN:.3f} is under 0.5 clear of the"
                     f" bore end {CH_Y0:.2f}")
if _PAD_Y0_OPEN < SLOT_Y0 or PAD_Y1_LOCKED - TRAVEL > SLOT_Y1:
    raise SystemExit("v8.6: pad leaves its slot somewhere in the stroke")
if DIMPLE_LOCK_Y - _TAIL_OPEN < 2.5:
    raise SystemExit("v8.6: LOCKED dimple too near the blade tail")
...
sleeve = bevel(box("Sleeve_V86", 0, BOX_X, 0, BOX_Y, 0, BOX_Z), EDGE_BREAK)
...
if abs(_sy0) > 1e-4 or abs(_sy1 - BOX_Y) > 1e-4:
    raise SystemExit(f"v8.6: sleeve spans Y {_sy0:.4f}..{_sy1:.4f}, must be 0..{BOX_Y}")
...
# --- DRAWER ------------------------------------------------------------------------
drawer = bevel(box("Drawer_V86", DR_X0, DR_X1, DR_Y0, DR_Y1, DR_Z0, DR_Z1), EDGE_BREAK)
...
bolt = bevel(box("Bolt_V86", BOLT_X0, BOLT_X1, TAIL_LOCKED, TIP_LOCKED, BOLT_Z0, BOLT_Z1),
             EDGE_BREAK)
...
# --- v8.6 MARK: the version, debossed on the bolt ----------------------------------
...
from watermark import _mesh_volume, _world_mesh_bbox, engrave  # noqa: E402

MARK_TEXT = "v8.6"                                     # the version alone: see the docstring
MARK_FACE, MARK_ROT = "-Z", 90.0                       # the pad land, read along the bolt
MARK_DEPTH, MARK_CAP, MARK_MARGIN = 0.6, 4.0, 1.0      # 0.6 = three layers, see the docstring
# FROZEN BOLT. v8.6 changes no geometry, so these are what v8.5 actually built
# (print/v8_5_pillbox_bolt.stl, Blender 4.5.12) and the mark's own cut as measured on the
# first v8.6 build. They are asserted below, before and after the mark.
BOLT_V85_DIMS = (8.0, 44.0, 10.0)
BOLT_V85_VOL = 2426.77695
MARK_CUT_MM3 = 7.96596
...
MARK = engrave(bolt, MARK_TEXT, face=MARK_FACE, depth=MARK_DEPTH, cap_mm=MARK_CAP,
               mode="deboss", margin_mm=MARK_MARGIN, rotate_deg=MARK_ROT)
...
for _name, _got, _want, _tol in [
    ...
    if abs(_got - _want) > _tol:
        raise SystemExit(f"v8.6: {_name} is {_got:.5f}, must be {_want:.5f}"
                         f" +/-{_tol:g} — v8.6 may not change the bolt, only mark it")
if MARK["warnings"]:
    raise SystemExit(f"v8.6: the mark is below a legibility floor: {MARK['warnings']}")
...
def report():
    print("\nv8.6 Y-STACK (mm, along the bolt axis)")
    for label, y, note in [
    ...
    print("\nv8.6 LOCKED FREE PLAY (mm of drawer movement, v8.2 -> v8.5)")
    ...
    print("\nv8.6 BRIDGED FACES (flat span that must bridge, mm)")
    ...
    print(f"   cap {MARK_CAP:.1f}mm, depth {MARK_DEPTH:.1f}mm ({MARK_DEPTH / 0.2:.0f} layers"
    ...
    print(f"   cut {_mark_cut:.3f}mm3 of {_pre_vol:.3f}mm3; bounding box unchanged at"
          f" {_post_dims[0]:.3f} x {_post_dims[1]:.3f} x {_post_dims[2]:.3f}"
...
# --- export ------------------------------------------------------------------------
# v8.6's DEFAULT PLATE IS THE BOLT ALONE. The project slices print/v8_6_pillbox_plate.stl,
# with no --only, straight from the dashboard, so whatever that file holds is what gets
# printed by "slice v8.6" — and the only thing v8.6 changes is the bolt. The drawer and
...
if not bpy.data.objects:
    raise SystemExit(f"v8.6: no part matches --only {ONLY!r}")
...
for o in bpy.data.objects:
    ...
    if max(abs(x0), abs(x1), abs(y0), abs(y1)) > PLATE_LIMIT:
        raise SystemExit(f"v8.6: {o.name} leaves the +/-{PLATE_LIMIT}mm plate window")
...
for _a in range(len(_parts)):
    for _b in range(_a + 1, len(_parts)):
        ...
        if gap < PART_GAP:
            raise SystemExit(f"v8.6: {_parts[_a].name}/{_parts[_b].name} gap {gap:.2f}"
                             f" < {PART_GAP}mm")

name = f"v8_6_pillbox_{ONLY.lower()}.stl" if ONLY else "v8_6_pillbox_plate.stl"
